...
boundPairList :
	boundPair
	{
		...
		if (tempNode->semTypeDef==storeBoundPairList) {  
			...
			newNode->dim = 1;
			newNode->lowerBound[dim-1] = tempNode->lowerBound[0];
			newNode->upperBound[dim-1] = tempNode->upperBound[0];
		}
		$$ = newNode;
	...
	boundPairList TOKEN_COMMA boundPair
	{
		...
		if (tempNodeOne->semTypeDef==storeBoundPairList && tempNodeTwo->semTypeDef==storeBoundPairList ){
			...
			newNode->lowerBound[dim-1] = tempNodeTwo->lowerBound[0];
			newNode->upperBound[dim-1] = tempNodeTwo->upperBound[0]; 	
		}
		else {
...
arraySegment :
	arrayIdentifier TOKEN_OPEN_SQUARE_BRACKET boundPairList TOKEN_CLOSE_SQUARE_BRACKET
	{
		...
		Node *tempNodeTwo = $3;
		currentScope = scopeStack[scopeStackTop-1];
		Symbol* entry = lookUp(tempNodeOne->identLex,currentScope);
		if(entry==NULL){
		...
		for(int i=0;i<dim;i++){
			...
			size = size*(upperBound[i]-lowerBound[i]+1);
		}
		entry->offset = symbolTable[currentScope].arrayOffset;
		symbolTable[currentScope].arrayOffset-=size;
...
procedureBody :
	TOKEN_BEGIN TOKEN_SEMICOLON statement TOKEN_END{//changed 16th april
		Node *new = createNode();
